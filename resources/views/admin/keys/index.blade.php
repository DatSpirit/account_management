    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-4 text-white">
                ...
                <div class="text-xs opacity-90">Tổng Keys</div>
            </div>
            ...
            <div class="bg-gradient-to-br from-gray-500 to-gray-600 rounded-xl p-4 text-white">
                ...
                <div class="text-xs opacity-90">Expired</div>
            </div>
            <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-4 text-white">
                ...
                <div class="text-xs opacity-90">Suspended</div>
            </div>
            <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl p-4 text-white">
                <div class="text-2xl font-bold">{{ $stats['deleted'] }}</div>
                <div class="text-xs opacity-90"> Đã xóa</div>
            </div>
        </div>
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div>
                    <select name="show_deleted"
                        ...
                        <option value="with" {{ request('show_deleted') == 'with' ? 'selected' : '' }}>Tất cả Key</option>
                        <option value="only" {{ request('show_deleted') == 'only' ? 'selected' : '' }}>Key đã xóa</option>
                    </select>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-gray-500 uppercase bg-gray-50 dark:bg-gray-700/50 dark:text-gray-400">
                        <tr>
                            <th class="px-6 py-4 font-semibold">ID / Thời gian</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($keys as $key)
                            <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition {{ $key->trashed() ? 'opacity-60' : '' }}">
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @if ($key->key_type == 'custom')
                                        <span class="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-purple-200 dark:border-purple-800">Custom</span>
                                    @else
                                        <span class="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-semibold px-2.5 py-0.5 rounded-full border border-blue-200 dark:border-blue-800">Auto</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if ($key->isActive())
                                        <span class="inline-flex items-center gap-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-bold px-2.5 py-0.5 rounded-full border border-green-200 dark:border-green-800">
                                            <span class="w-1.5 h-1.5 bg-green-500 rounded-full"></span> Active
                                        </span>
                                    @elseif($key->isExpired())
                                        <span class="inline-flex items-center gap-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs font-bold px-2.5 py-0.5 rounded-full border border-gray-300">
                                            Expired
                                        </span>
                                    @elseif($key->isSuspended())
                                        <span class="inline-flex items-center gap-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-bold px-2.5 py-0.5 rounded-full border border-red-200 dark:border-red-800">
                                            Suspended
                                        </span>
                                    @endif
                                </td>
                    ...
                    <div class="absolute top-4 right-4 z-10">
                        ...
                        @endif
                                ...
                                <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                                    @if ($key->expires_at)
                                        <div>{{ $key->expires_at->setTimezone('Asia/Ho_Chi_Minh')->format('d/m/Y') }}</div>
                                        <div>{{ $key->expires_at->setTimezone('Asia/Ho_Chi_Minh')->format('H:i:s') }}</div>
                                        <div class="text-xs text-gray-400">{{ $key->expires_at->setTimezone('Asia/Ho_Chi_Minh')->diffForHumans() }}</div>
                                    @else
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        @if($key->trashed())
                                            ...
                                            <form action="{{ route('admin.keys.restore', $key->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                        ...
                                        @else
                                            ...
                                            {{-- Modal xác nhận xóa --}}
                                            <div id="deleteModal{{ $key->id }}" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                                                    ...
                                                    <div class="p-6">
                                                        ...
                                                        <div class="space-y-3 mb-6">
                                                            <div class="flex items-start gap-3">
                                                                ...
                                                                <div>
                                                                    <p class="font-semibold text-gray-900 dark:text-white">Key Code: <span class="font-mono text-red-600">{{ $key->key_code }}</span></p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="flex items-start gap-3">
                                                                ...
                                                                <div>
                                                                    <p class="text-sm text-gray-700 dark:text-gray-300">Admin vẫn thấy key với nhãn <span class="px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded font-semibold">ĐÃ XÓA</span></p>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div class="flex items-start gap-3">
                                                                ...
                                                                <div>
                                                                    <p class="text-sm text-gray-700 dark:text-gray-300">Có thể <strong class="text-green-600">khôi phục</strong> key này bất cứ lúc nào</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <form action="{{ route('admin.keys.destroy', $key->id) }}" method="POST">
                                                            ...
                                                            <div class="flex gap-3">
                                                                <button type="button" onclick="closeDeleteModal{{ $key->id }}()"
